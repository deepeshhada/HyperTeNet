...
from models.transformer import TransformerModelMine
...
class HyperTeNet(torch.nn.Module):
    def __init__(self, params, device='cuda:0'):
        ...
        # embedding matrices
        self.user_list_item_embeddings = torch.nn.Embedding(params.num_user + params.num_list + params.num_item,
                                                            params.num_factors)
        ...
        self.user_item_list_dropout = torch.nn.Dropout(1.0 - params.keep_prob)
        self.dropout1 = torch.nn.Dropout(1.0 - params.keep_prob)
        self.dropout2 = torch.nn.Dropout(1.0 - params.keep_prob)
        self.sigmoid = torch.nn.Sigmoid()

        torch.nn.init.xavier_normal_(self.user_list_item_embeddings.weight)
        torch.nn.init.zeros_(
            self.user_list_item_embeddings.weight[0])
        ...
        self.user_conv1 = GCNConv(params.num_factors, params.hid_units[-2], cached=True, normalize=True,
                                  add_self_loops=True)
        self.user_conv2 = GCNConv(params.hid_units[-2], params.hid_units[-1], cached=True, normalize=True,
                                  add_self_loops=True)

        self.list_conv1 = GCNConv(params.num_factors, params.hid_units[-2], cached=True, normalize=True,
                                  add_self_loops=True)
        self.list_conv2 = GCNConv(params.hid_units[-2], params.hid_units[-1], cached=True, normalize=True,
                                  add_self_loops=True)

        self.item_conv1 = GCNConv(params.num_factors, params.hid_units[-2], cached=True, normalize=True,
                                  add_self_loops=True)
        self.item_conv2 = GCNConv(params.hid_units[-2], params.hid_units[-1], cached=True, normalize=True,
                                  add_self_loops=True)
        ...
        if params.args.knn_graph == 'True':
            ...
            self.user_param_weights, self.list_param_weights, self.item_param_weights = None, None, None
        else:
            self.user_adj_mat = params.user_adj_mat.tocoo()
            self.user_adj_mat.setdiag(0)
            self.user_adj_mat.eliminate_zeros()
            self.user_param_indices = torch.LongTensor(self.user_adj_mat.nonzero()).to(device)
            self.user_param_weights = torch.FloatTensor(self.user_adj_mat.data).to(device)

            self.list_adj_mat = params.list_adj_mat.tocoo()
            self.list_adj_mat.setdiag(0)
            self.list_adj_mat.eliminate_zeros()
            self.list_param_indices = torch.LongTensor(self.list_adj_mat.nonzero()).to(device)
            self.list_param_weights = torch.FloatTensor(self.list_adj_mat.data).to(device)

            self.item_adj_mat = params.item_adj_mat.tocoo()
            self.item_adj_mat.setdiag(0)
            self.item_adj_mat.eliminate_zeros()
            self.item_param_indices = torch.LongTensor(self.item_adj_mat.nonzero()).to(device)
            self.item_param_weights = torch.FloatTensor(self.item_adj_mat.data).to(device)
            if params.args.user_adj_weights == 'False':
                self.user_param_weights, self.list_param_weights, self.item_param_weights = None, None, None

        # dropouts gnn part
        self.user_gnn_dropout = torch.nn.Dropout(1.0 - params.gnn_keep_prob)
        self.list_gnn_dropout = torch.nn.Dropout(1.0 - params.gnn_keep_prob)
        self.item_gnn_dropout = torch.nn.Dropout(1.0 - params.gnn_keep_prob)

        # seq part ============================================================================
        self.pos_embeddings = torch.nn.Embedding(params.max_item_seq_length, params.hid_units[-1])

        self.user_dropout = torch.nn.Dropout(1.0 - params.keep_prob)
        self.list_dropout = torch.nn.Dropout(1.0 - params.keep_prob)
        self.item_dropout = torch.nn.Dropout(1.0 - params.keep_prob)
        self.item_dropout1 = torch.nn.Dropout(1.0 - params.keep_prob)
        self.item_dropout2 = torch.nn.Dropout(1.0 - params.keep_prob)
        self.item_dropout3 = torch.nn.Dropout(1.0 - params.keep_prob)
        self.item_dropout4 = torch.nn.Dropout(1.0 - params.keep_prob)
        self.item_dropout5 = torch.nn.Dropout(1.0 - params.keep_prob)
        self.item_dropout6 = torch.nn.Dropout(1.0 - params.keep_prob)
        self.item_dropout7 = torch.nn.Dropout(1.0 - params.keep_prob)
        self.pos_dropout = torch.nn.Dropout(1.0 - params.keep_prob)

        self.sigmoid_seq = torch.nn.Sigmoid()

        # transormer model ======================
        self.trans_model = TransformerModelMine(ntoken=params.num_item, ninp=params.hid_units[-1],
                                                nhead=params.n_heads[-1],
                                                nhid=params.hid_units[-1], nlayers=params.num_layers, dropout=0.3)
        self.layer_norm = nn.LayerNorm(params.hid_units[-1])

        # hgnn ==================================
        self.hypersagnn_model = HyperSAGNN(n_head=params.n_heads[0], d_model=params.hid_units[-1],
                                           d_k=params.hid_units[-1], d_v=params.hid_units[-1],
    ...
    def forward(self, user_indices, list_indices, item_indices=None, item_seq=None, item_seq_pos=None,
        ...
        # seq ===================================
            ...
            if train:
                ...
                self.user_item_seq_pos_embeds = self.user_embeds.reshape(-1, 1, self.params.hid_units[
                    -1]) * self.item_seq_pos_embeds
                self.list_item_seq_pos_embeds = self.list_embeds.reshape(-1, 1, self.params.hid_units[
                    -1]) * self.item_seq_pos_embeds
                self.item_seq_and_seq_pos_embeds = self.item_seq_embeds * self.item_seq_pos_embeds

                self.user_item_seq_neg_embeds = self.user_embeds.reshape(-1, 1, self.params.hid_units[
                    -1]) * self.item_seq_neg_embeds
                self.list_item_seq_neg_embeds = self.list_embeds.reshape(-1, 1, self.params.hid_units[
                    -1]) * self.item_seq_neg_embeds
                self.item_seq_and_seq_neg_embeds = self.item_seq_embeds * self.item_seq_neg_embeds

                self.pos_logits = self.sigmoid(torch.sum(
                    self.user_item_seq_pos_embeds + self.list_item_seq_pos_embeds + self.item_seq_and_seq_pos_embeds,
                    axis=-1))
                self.neg_logits = self.sigmoid(torch.sum(
                    self.user_item_seq_neg_embeds + self.list_item_seq_neg_embeds + self.item_seq_and_seq_neg_embeds,
                    axis=-1))
                ...
                self.item_seq_embeds = self.item_seq_embeds.view(-1, 1, self.params.max_item_seq_length,
                    ...
                    -1, self.params.max_item_seq_length, self.params.hid_units[-1])
                self.list_embeds = self.list_embeds.view(-1, 1, self.params.hid_units[-1]).repeat(1, 101, 1).view(-1,
                                                                                                                  self.params.hid_units[
                                                                                                                      -1])
                self.user_embeds = self.user_embeds.view(-1, 1, self.params.hid_units[-1]).repeat(1, 101, 1).view(-1,
                                                                                                                  self.params.hid_units[
                                                                                                                      -1])

                self.pos_logits = self.sigmoid(torch.sum(
                    (self.item_seq_embeds[:, -1, :] + self.list_embeds + self.user_embeds) * self.test_item_embeds,
                    axis=-1))

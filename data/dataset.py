...
class Dataset(object):
    def __init__(self, args):
        ...
        self.num_user, self.num_list, self.num_item = self.get_user_list_item_count(self.user_lists_dct,
                                                                                    self.list_items_dct)
        self.list_user_vec = self.get_list_user_vec(self.list_user_dct,
                                                    self.num_list)  # user-embedding can be obtained using this(many-to-one)
        ...
        self.train_matrix_item_seq = self.get_dct_mat_seq_remove_test(dct=self.list_items_dct, num_row=self.num_list,
                                                                      num_col=self.max_item_seq_length + 1,
                                                                      padding_value=0)
        self.train_matrix_item_seq_for_test = self.get_dct_mat_seq_for_test(dct=self.list_items_dct,
                                                                            num_row=self.num_list,
                                                                            num_col=self.max_item_seq_length,
                                                                            padding_value=0)
        print("avg lists per user:", sum([len(l) for l in list(self.user_lists_dct.values())]) / len(
    ...
    def get_user_list_item_count(self, user_lists_dct, list_items_dct):
        ...
        assert len(list_items_dct.keys()) == len(set(itertools.chain.from_iterable(
            self.user_lists_dct.values()))), 'number of lists from both the list_items_dct and user_lists_dct should be same.'
    ...
    def get_train_matrix_sp(self, dct, num_row, num_col):
        mat = sp.dok_matrix((num_row, num_col), dtype=np.float16)
        for key in dct:
            for val in dct[key]:
                mat[key, val] = 1.0
        return mat.tolil()
